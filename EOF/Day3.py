...
from Colors import Arstotzka, Obristan, Kolechia, Impor, UnitedFed, Republia, Antegria, black, LadyCard, LadyCardSecondary, PersonColor, TextBoxColor, TicketColor, TicketColor2
...
while dayStat == day:
    ...
    if PersonPresent and TextBox == TextBoxColor:
        ...
        if VisTicket == TicketColor or TicketColor2:
            # MOVE TICKET
    ...
    if NoPassport:
        ...
        # NO PASSPORT RULE BROKEN
        pyautogui.moveTo(RuleBookBasicsX, RuleBookBasicsY)
        pyautogui.click()

        pyautogui.moveTo(InspectButtonX, InspectButtonY)
        pyautogui.click()

        pyautogui.moveTo(PassportRuleX, PassportRuleY)
        pyautogui.click()
        ...
        time.sleep(1.5)
        pyautogui.moveTo(InterrogateX, InterrogateY)
        pyautogui.click()
        ...
        #time.sleep(1.5)
        #pyautogui.moveTo(TempSlipX, TempSlipY)
        #pyautogui.mouseDown(button='left')
    ...
    if StampingTime:
        ...
        if TicketPresent:
            ...
            pyautogui.moveTo(TicketDateX, TicketDateY)
            pyautogui.click()
            ...
            cap = ImageGrab.grab(
                bbox=(TicketDateInspectTopX, TicketDateInspectTopY, TicketDateInspectBottomX, TicketDateInspectBottomY))

            # Converted the image to monochrome for it to be easily
            # read by the OCR and obtained the output String.
            TicketDate = pytesseract.image_to_string(
                cv2.cvtColor(nm.array(cap), cv2.COLOR_BGR2GRAY),
                lang='eng')

            TicketDate = TicketDate.strip()
            print(TicketDate)

            similarity_ratio = Levenshtein.ratio(TicketDate, Matching)
    ...
    if ValidTicket and TicketPresent:
        # MOVE RULE
        pyautogui.moveTo(RuleBookX, RuleBookY)
        pyautogui.mouseDown(button='left')
        pyautogui.moveTo(RuleBookSlotX, RuleBookSlotY)
    ...
    elif StampingTime and TicketPresent:
        print("FOREIGN SCUM")
